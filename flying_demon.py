...
class FlyingDeamon(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.flying_frames = [
            pygame.transform.scale(
                pygame.image.load(f"./assets/flying_demon/demon_flying_{i}.png"),
        ...
        self.attack_frames = [
            pygame.transform.scale(
                pygame.image.load(f"./assets/flying_demon/demon_attack_{i}.png"),
                (self.height, self.width),
            )
            for i in range(8)
        ]
        self.bk_flying_frames = [
            pygame.transform.scale(
        ...
        self.hurt_frame = pygame.transform.scale(
            pygame.image.load(f"./assets/flying_demon/hurt_1.png"),
            (self.height, self.width),
        )
        self.bk_hurt_frame = pygame.transform.scale(
            pygame.transform.flip(
                pygame.image.load(f"./assets/flying_demon/hurt_1.png"), True, False
            ),
            (self.height, self.width),
        )
        self.projectile = pygame.transform.scale(
            pygame.image.load(f"./assets/flying_demon/projectile.png"), (48, 32)
        )
        self.projectile_rect = self.projectile.get_rect()
        self.projectile_rect.center = (self.projectile_x, self.projectile_y)
        self.bk_projectile = pygame.transform.scale(
            pygame.transform.flip(
                pygame.image.load(f"./assets/flying_demon/projectile.png"), True, False
            ),
            (48, 32),
        )
    ...
    def attack(self, screen, hero):
        if not self.is_it_stop:
            ...
            if hero.mask.overlap(
                self.mask_projectile,
                (
                    self.projectile_rect.x - hero.rect.x,
                    self.projectile_rect.y - hero.rect.y,
                ),
            ):
                hero.hit = True
                self.projectile_rect.x = int((screen_width / 2) - 30)
            ...
            if hero.mask.overlap(
                self.bk_mask_projectile,
                (
                    self.bk_projectile_rect.x - hero.rect.x,
                    self.bk_projectile_rect.y - hero.rect.y,
                ),
            ):
                hero.hit = True
                self.bk_projectile_rect.x = int(screen_width / 2)
    ...
    def hurt(self, facing_forward, sword_rect, keys):
        ...
        arrow_keys_pressed = (
            keys[pygame.K_LEFT] or keys[pygame.K_RIGHT] or keys[pygame.K_n]
        )

        if (
            self.rect.colliderect(sword_rect)
            and keys[pygame.K_SPACE]
            and not arrow_keys_pressed
        ):
            if facing_forward:
